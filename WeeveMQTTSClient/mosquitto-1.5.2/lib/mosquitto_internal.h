...
#if defined(WITH_WEEVE_SMP)
	uint8_t *smp_payload;
	WosBuffer_t mqttPacket;
	uint32_t smp_remaining_mult;
	uint32_t smp_remaining_length;
	uint32_t smp_packet_length;
	uint32_t smp_to_process;
	uint32_t smp_pos;
	int8_t smp_remaining_count;
#endif
};
